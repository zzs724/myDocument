...
- **配置.conf文件：**
  ...
  - **cluster-config-file  nodes-6379.conf：**nodes信息配置文件
  - **cluster-require-full-coverage no：**是否需要每个节点都可用，集群才算可用

- **启动每个端口的redis服务：**  /usr/local/bin/redis-server  cluster7000.conf

- **查看集群信息：**

  - **/usr/local/bin/redis-cli  -p 7000 cluster slots：**查看槽信息
  - **/usr/local/bin/redis-cli  -p 7000 cluster info：**查看集群信息

- **集群之间相互握手/建立通信**

  - **/usr/local/bin/redis-cli  -p 7000 meet  IP  Port：**只需一台meet所有的就可以了

- **集群中设置主从：**

  - **/usr/local/bin/redis-cli  -p 7000 cluster nodes：**查处集群中的redis服务ID
  - **/usr/local/bin/redis-cli  -p 7003 cluster replicate  Reids的ID：**7003设置为Reids的ID的从节点

- **分配slots（槽）**：

  - 编写shell脚本：addslots.sh
  ...
  - 执行shell脚本：sh addslots.sh 10923 16383 7002
